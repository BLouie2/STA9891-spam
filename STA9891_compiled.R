...
cat("\014")
library(class); library(ggplot2)
library(dplyr); library(doMC) # parallel computing
library(tidyverse); library(glmnet)
library(ISLR); library(readr)
library(randomForest); library(visdat) 
library(AUC); library(tm) # for stopwords
library(ggthemes); library(latex2exp)
library(comprehenr); library(gridExtra)
...
lsPlot.spam.feat <-  ggplot(lasso.betas.df,aes(x= fct_rev(reorder(feature, -obh)),
                                               y=value, fill = pos)) +
  geom_col(position = "identity", width = 3) + ylab('coefs') + 
